...
public class Sales extends JFrame {
	...
	private JLabel getLblNewLabel_1() {
		if (lblNewLabel_1 == null) {
			...
			lblNewLabel_1.setBounds(332, 95, 81, 14);
		}
	...
	private JTable getTable() {
		if (table == null) {
			...
			table.setModel(new DefaultTableModel(
				...
				new String[] {
					"Bill No", "Customer Name", "Amount","Quantity","Date"
				}
			));
		}
		return table;
	...
	private void displayData() {
		...
		for(Bill c: list) {
			tmodel.addRow(new Object[] {c.getBillNo(),c.getCustomerName(),c.getTotal(),c.getQuantity(),c.getDate()});
		}
	}
	...
	private JTextField getSerach() {
		if (serach == null) {
			...
			serach.addKeyListener(new KeyAdapter() {
				...
				for(Bill s: slist) {
					tmodel.addRow(new Object[] {s.getBillNo(),s.getCustomerName(),s.getTotal(),s.getQuantity(),s.getDate()});
				}
					
				}
			});
			serach.setBounds(408, 90, 197, 31);
